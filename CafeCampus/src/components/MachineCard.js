...
// ── SVG illustrations ────────────────────────────────────────
// Machine 1 : expresso classique (couleur bordeaux)
const SVG_MACHINE_1 = `
...
// Machine 2 : machine à capsules (couleur plus claire)
const SVG_MACHINE_2 = `
<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <!-- Corps principal -->
...
export default function MachineCard({ machine, onEdit, onSave, onCancel, isEditing, isSaving }) {
  ...
  // Choisit l'illustration selon l'ID ou le nom de la machine
  const illustration = machine.id === 'machine_2' || machine.name?.includes('2')
    ? SVG_MACHINE_2
    : SVG_MACHINE_1;
    ...
    <View style={styles.card}>
      ...
      <View style={styles.illustrationRow}>
        ...
        <View style={styles.illustrationWrapper}>
          <SvgXml xml={illustration} width={110} height={110} />
        </View>

        {/* Colonne droite : timestamp + statuts */}
...
const styles = StyleSheet.create({
  ...
  machineName: {
    color: COLORS.white, fontWeight: '700', fontSize: 15, fontStyle: 'italic',
  },
  machineLocation: {
    color: COLORS.accentDark, fontSize: 12, marginTop: 1,
  },
  ...
  notesLabel: {
    fontWeight: '700', fontSize: 13, color: COLORS.black, marginBottom: 4,
  },
  notesText: {
    fontSize: 13, color: COLORS.black, lineHeight: 18,
  },
  notesEmpty: {
  ...
  charCount: {
    textAlign: 'right', fontSize: 11, color: COLORS.grey, marginTop: 4,
  },
  charCountWarn: { color: COLORS.horsServiceText },

  // Actions
  actionRow: {
    flexDirection: 'row', marginHorizontal: 16, marginBottom: 14, gap: 10,
  },
  cancelButton: {
